...
Each note gets its own filter sweep, starting when you play it. This wording is reconstructed from the shipped patch metadata. The current implementation is the retrospective baseline: it must remain stable, audible, and distinguishable from every other exported preset.

## The one committed target

Preserve the specific “Each note gets its own filter sweep, starting when you play it.” character rather than smoothing it into a general-purpose pluck sound; a broader but less recognizable version loses.
