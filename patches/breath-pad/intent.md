...
Pink noise blended under the strings — the air a real ensemble has. This wording is reconstructed from the shipped patch metadata. The current implementation is the retrospective baseline: it must remain stable, audible, and distinguishable from every other exported preset.

## The one committed target

Preserve the specific “Pink noise blended under the strings — the air a real ensemble has.” character rather than smoothing it into a general-purpose pad sound; a broader but less recognizable version loses.
